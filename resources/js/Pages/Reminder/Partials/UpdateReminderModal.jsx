...
export default function UpdateReminderModal({ show, onClose, reminder }) {
    ...
    useEffect(() => {
        if (reminder) {
            setData({
                ...
                event_at: toDatetimeLocal(reminder.event_at),
                remind_at: toDatetimeLocal(reminder.remind_at),
                roles: reminder.roles ?? [],
                notify_self: reminder.notify_self ?? false,
                people_ids: reminder.recipients?.map((p) => p.id) ?? [],
                notification_methods: reminder.notification_methods ?? [],
            });
            setSelectedPeople(reminder.recipients ?? []);
        }
    }, [reminder]);
    ...
    const submit = (event) => {
        ...
        put(route('reminder.update', reminder.id), { onSuccess: close });
    };
        ...
        <Modal show={show} onClose={close} maxWidth="3xl">
            <form onSubmit={submit} className="max-h-[85vh] overflow-y-auto p-6">
                <div className="flex items-start justify-between gap-4">
                    <h2 className="text-lg font-medium text-gray-900">Update Reminder</h2>
                    <button
                        type="button"
                        onClick={close}
                        aria-label="Close"
                        ...
                        <div className="mt-3 space-y-4">
                            <div>
                                <div className="flex items-center gap-1">
                                    <InputLabel htmlFor="update-event-name" value="Event name" />
                            ...
                            <div>
                                <InputLabel htmlFor="update-event-description" value="Event description" />
                                <textarea
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div>
                                    <InputLabel htmlFor="update-event-at" value="Event date & time" />
                                    <TextInput
                                        id="update-event-at"
                                        type="datetime-local"
                                        className="mt-1 block w-full"
                                ...
                                <div>
                                    <div className="flex items-center gap-1">
                                        <InputLabel htmlFor="update-remind-at" value="Reminder date & time" />
                                        <span className="text-red-500" aria-hidden="true">*</span>
                                    </div>
                                    <TextInput
                                        id="update-remind-at"
